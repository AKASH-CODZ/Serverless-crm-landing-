...
interface AgeGroupAutocompleteProps {
  value: string;
...
export function AgeGroupAutocomplete({
  value,
  onChange,
  placeholder = 'Select an age group...',
  required = false,
  accentColor = '#00E5FF',
  id = 'age-group-autocomplete',
  name = 'ageGroup',
}: AgeGroupAutocompleteProps) {
  ...
  const filteredSuggestions = AGE_GROUP_OPTIONS.filter((option) =>
    ...
    <div ref={containerRef} className="relative w-full">
      ...
      <div className="relative">
        <input
          ...
          required={required}
          aria-label="Age group autocomplete"
          aria-autocomplete="list"
          aria-controls={`${id}-listbox`}
          aria-expanded={isOpen}
          className="w-full px-4 py-3 rounded-[10px] bg-white/5 border border-white/10 focus:border-[#00E5FF] focus:outline-none transition-colors pr-10"
        />
        {/* Chevron Icon */}
        <div
